#!/usr/bin/env python3
"""
AI Daily Push — 从 DailyBrief 每日报告中提取 AI 相关 TOP20 新闻，
生成 markdown 文件并推送到 GitHub AIDaily 仓库。
"""
import json
import os
import subprocess
...
def is_ai_brief(brief):
    """Check if a tech brief is AI-related."""
...
def generate_summary(report_json_path, ai_articles):
    """Generate a ~100 word summary from the report's AI briefs and overview."""
    summaries = []
    
    # Try to get AI-related tech briefs from the report JSON
    if report_json_path:
        try:
            with open(report_json_path, 'r', encoding='utf-8') as f:
                report = json.load(f)
            
            # Collect AI-related brief summaries
            ...
            # If editor_note mentions AI, include it
    ...
    # Fallback: build summary from top articles' excerpts
    if not summaries:
    ...
    # Combine and truncate to ~100 Chinese chars (≈150 chars to be safe)
    combined = '；'.join(summaries)
    if len(combined) > 150:
        combined = combined[:147] + '...'
    
    return combined if combined else '今日AI领域新闻精选。'

def generate_markdown(date_str, articles, summary):
    """Generate a markdown file with summary + top 20 AI articles."""
    lines = [
        f"# 🤖 AI Daily — {date_str}",
        "",
        f"## 📝 今日摘要",
